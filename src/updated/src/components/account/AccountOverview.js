...
export default class AccountOverview extends Component {
    ...
    computeClass() {
        return this.state.overview.earnings >= 0 ? ' positive ' : ' negative '
    }
    ...
    async getAccountOverview() {
        try {
            ...
            const response = await fetch(
                CoreConstants.ApiUrls.AccountOverview
            )
                                        ...
                                        <div className="column is-12">
                                            ...
                                            <span className="icon-text net">
                                                <span className={"icon " + (this.computeClass())}>
                                                    {
                                                        this.state.overview.earnings >= 0 ?
                                                            <TbTrendingUp/> :
                                                            <TbTrendingDown />
                                                    }
                                                </span>
                                                <span className={this.computeClass()}>
                                                    {formatNumberForDisplay(this.state.overview.earnings)}
                                                </span>
                                            </span>
                                        </div>
                                        <div className="column is-12">
