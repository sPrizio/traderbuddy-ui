...
export default class ForecastingPage extends Component {
    ...
    displayDate(val) {
        let dateDisplay = <span className="value">{moment(val).format(CoreConstants.DateTime.ISOYearFormat)}</span>
        if (this.state.selectedInterval === 'DAILY') {
            const day = moment(val).format(CoreConstants.DateTime.ISODayFormat).match('(\\d+)([a-zA-z]+)')
            dateDisplay =
                <div>
                    <span className="sub-header">{moment(val).format(CoreConstants.DateTime.ISOMonthFormat)}</span>
                    <br/>
            ...
            dateDisplay = <span className="value">{moment(val).format(CoreConstants.DateTime.ISOMonthFormat)}</span>
        }
    ...
    render() {
        return (
            <>
                <Helmet>
                    <title>TraderBuddy | Forecasting & Performance</title>
                </Helmet>
