        ...
        try {
            ...
            const response = await fetch(
                CoreConstants.ApiUrls.Account.EquityCurve
                    .replace('{start}', moment().subtract(this.props.count, 'months').startOf('month').format(CoreConstants.DateTime.ISODateFormat))
                    .replace('{end}', moment().add(1, 'months').startOf('month').add(1, 'days').format(CoreConstants.DateTime.ISODateFormat))
